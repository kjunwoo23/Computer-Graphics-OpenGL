...
void myDisplay()
{
	...
	//mat4 ProjM = myOrtho(left, right, bottom, top, near, far);	Quantization(����ȭ)
	// aspect ratio (��Ⱦ��) : w/h 16/9 4/3
...
void myKeyboard(unsigned char c, int x, int  y)
{
	switch (c) 
	{
	case '1': g_size += 0.1f;		glutPostRedisplay(); break;
	case '2': g_size -= 0.1f;		glutPostRedisplay(); break;
	default:											 break;
	}
}
